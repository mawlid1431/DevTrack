...
const demoDataReturns = v.object({
  teams: v.number(),
  labels: v.number(),
  projects: v.number(),
  cycles: v.number(),
...
async function seedDemoWorkspace(
  ...
  user: Doc<"users">
    ...
    for (const comment of COMMENTS) {
      ...
      const body = comment.mention ? `@${user.name} ${comment.body}` : comment.body;
...
export const demoData = orgAdminMutation({
  args: {},
  returns: demoDataReturns,
  handler: async (ctx) => seedDemoWorkspace(ctx, ctx.org, ctx.user),
});

/** Dev/bootstrap helper — seeds demo data for an org by Clerk org id (no JWT). */
export const bootstrapDemoData = internalMutation({
  ...
  handler: async (ctx, { clerkOrgId }) => {
    ...
    const membership = await ctx.db
      ...
      .first();
    if (!membership) {
      throw new Error(`No members found for org: ${clerkOrgId}`);
    }
    const user = await ctx.db.get(membership.userId);
    if (!user) {
      throw new Error(`Member user not found for org: ${clerkOrgId}`);
    }
    return await seedDemoWorkspace(ctx, org, user);
  },
});
